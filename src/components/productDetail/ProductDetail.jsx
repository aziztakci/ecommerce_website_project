    ...
    <section className="bg-light-gray-1">
      <PageContent className="detailcontainer flex flex-col md:flex-row gap-7.5 justify-center md:gap-8.5 max-w-272.5 px-5">
        ...
        <div className="textcontainer flex flex-col mx-auto w-full md:w-128.75 px-6.5">
          <h4 className="font-normal text-text text-[20px] pt-2.5">
            {product.name}
          </h4>
          <div className="ratescore flex gap-2.5 pt-3">
            ...
            <h6 className="font-bold text-[14px] text-second-text ">
              ({product.sell_count} Reviews)
            </h6>
          </div>
          <h3 className="font-bold text-text text-[24px] pt-5">
            ${product.price}
          </h3>
          <h6 className="font-bold text-second-text text-[14px] pt-1.25">
            Availability:{" "}
            <span
              className={`font-bold text-[14px] ${product.stock > 0 ? "text-primary" : "text-alert"}`}
            >
              {product.stock > 0
          ...
          <p className="font-normal text-light-text-gray-3 pt-8 pb-4 md:pb-6.25">
            {product.description}
          </p>
          ...
          <div className="flex gap-2.5 items-center">
            <button
              ...
              className={`font-bold text-light-text text-[14px] leading-none px-5 py-4 rounded-[5px] transition-transform active:scale-95 ${
                product.stock > 0
                  ? "bg-primary"
            ...
            <div className="iconcontainer flex gap-2.5">
              <Heart
                ...
                className={`w-5.5 h-5.5 border border-light-gray-3 rounded-full p-2 box-content cursor-pointer transition-all duration-300 ${
                  isFavorite
                    ? "text-white bg-alert border-alert fill-white"
                    : "text-pure-black hover:bg-light-gray-2"
                }`}
              />

              <ShoppingCart
                onClick={toggleCart}
                className={`w-5.5 h-5.5 border border-light-gray-3 rounded-full p-2 box-content cursor-pointer hover:bg-light-gray-2 transition-colors ${
                  isInCart
                    ? "text-primary border-primary bg-light-gray-1"
                    : "text-pure-black"
                }`}
              />
            </div>
